; IntelliSense -- by Rajat (requires XP/2k/NT) modify by hoothin
...
Loop
{
	...
	{
		IfNotInString, ActiveTitle, sublime
		{
			IfNotInString, ActiveTitle, emacs
			{
				IfNotInString, ActiveTitle, vim
				{
					ToolTip
					Sleep, 500
					Continue
...
IfWinNotExist, AutoHotkey Help
{
	...
	clipboard = 
	IfInString, ActiveTitle, %I_Editor%
	{
		send ^{Left}
		Send ^+{Right}
		Send ^c
	}
	else IfInString, ActiveTitle, sublime
	{
		IfInString, ActiveTitle, .ahk
		{
			send {Esc 2}
			send i
			send ^d
	...
	else IfInString, ActiveTitle, vim
	{
		send {Esc 2}
		send y
		send a
	...
	else IfInString, ActiveTitle, emacs
	{
		send ^!b
		send !d
		send ^y
	...
	if I_HasCmd
		run keyhh.exe -Myhelp -#klink %clipboard% %ahk_help_file%
	else
		run hh %ahk_help_file%
	IfWinExist, AutoHotkey 中文帮助
		WinWait, AutoHotkey 中文帮助
	else IfWinExist, AutoHotkey Help
